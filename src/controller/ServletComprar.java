		...
		try {
			...
			if (sessao.getAttribute("carrinho") == null)
				...
				if (sessao.getAttribute("cpf") == null) {
					...
					// Para evitar pedido com valor zerado
					if (pedido.getTotal().compareTo(BigDecimal.ZERO) > 0) {
						...
						for (int i = 0; i < carrinho.size(); i++) {
							...
							total = Double.parseDouble(produtoCarrinho.getPreco_venda().toString())
									* produtoCarrinho.getQuantidade_carrinho();
							temp.setValor_total(new BigDecimal(total));

							// Adiciona o pedido caso o produto tenha em estoque
							if(produtoCarrinho.getQuantidade_carrinho() > 0){
							...
							produto = produtoDAO.retrieveProdutoID(produtoCarrinho.getId());

							atualizaEstoque = produto.getQuantidade_estoque()
									- produtoCarrinho.getQuantidade_carrinho();
							produto.setQuantidade_estoque(atualizaEstoque);
